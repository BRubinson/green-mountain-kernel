...
2. **gm_session_startup.sh executes** — a few jobs only: confirm we're in a git repository, hold the hook payload, find the plugin root, and find the `gm_hook` binary under the one runtime root, `$HOME/gmfs`. Everything else is owned by that binary.
