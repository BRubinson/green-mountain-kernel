---
name: personality
description: The methodology lenses a fan-out agent wears
...
`compliant` is the solo lens: one mind covering every angle when there is no fan-out. The other four — `aggressive`, `pragmatic`, `alternative`, `conservative` — are the party, spawned together so their disagreements surface as options rather than as one agent's preference.
