...
description: Standalone GM-CDE session bootstrapper. Ensures the current session's db rows (gm_hook context ensure) and physical artifact home (prompts/) exist, independent of the SessionStart hook. Idempotent — never clobbers existing state.
user-invocable: true
...
`SessionStart` hook (`scripts/gm_session_startup.sh`) already does this
...
**It does NOT modify `gm_session_startup.sh` or the SessionStart flow.**
...
- To recreate a deleted session artifact home (the `prompts/` directory under the session's `gmfs_relative_storage_path`).
...
Both steps are natively idempotent: `gm_hook context ensure` upserts (reusing
...
Verify `$GM_FS_ROOT` is set. If unset, the environment never booted —
...
Resolve the session's `gmfs_relative_storage_path` from the session row:

```bash
gm_hook call SESSION_GET --json '{"session_uuid":"<SESSION_UUID>"}'
mkdir -p "$GM_FS_ROOT/{gmfs_relative_storage_path}/prompts"
```

(a no-op when `gm_hook context ensure` already created it).
...
Session ready: {artifact home} (artifacts) + ~/gmfs/gm.db (data)
```
...
- Kbite inheritance is seeded db-side at row-create time by
  `gm_hook context ensure`. Explicit registry ops afterward are
