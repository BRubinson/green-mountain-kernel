...
If `$GM_BOOTED` is not set, stop with `[GMB] ERROR: GMCC not booted`
(run /gmcc_boot for diagnostics).

Invoke the `gmcc_cleanup_system` skill and follow it. The env-vs-db root
audit is `gm_hook context env --plugin-root "$GM_PLUGIN_ROOT"`: it
...
against `gm_hook paths --json` (the db's answer), then check `~/.zshrc`
...
The gmfs/db data audit (artifact drift, archive hygiene, kbite provenance)
is `/gmcc_cleanup`.
