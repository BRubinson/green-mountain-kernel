---
description: Load GMCC session context, then just do the task. Writes no prompt rows or report summaries — persistence happens only via the automatic file-change hook, an optional doper briefing for meaty tasks, or an explicitly requested retroactive write-back.
