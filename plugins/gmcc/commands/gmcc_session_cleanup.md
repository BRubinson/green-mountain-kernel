...
(`$GM_FS_ROOT/{session gmfs_relative_storage_path}`, from
`gm_hook call SESSION_GET --json '{"session_uuid":"{U}"}'`) vs this
...
`$GM_PLUGIN_ROOT/skills/gmcc_session_cleanup/SKILL.md`.
...
Verify the session's artifact home exists: `gm_hook context ensure` gives
the session uuid, then resolve
`$GM_FS_ROOT/{gmfs_relative_storage_path}` from
`gm_hook call SESSION_GET --json '{"session_uuid":"{U}"}'`. If not, the environment never
...
(`$GM_PLUGIN_ROOT/skills/gmcc_session_cleanup/SKILL.md`) for the complete
session-scoped walk strategy and finding categories.

Follow that skill's protocol:

1. Health first: `gm_hook ping`, `gm_hook context ensure`.
...
5. Apply the user's chosen action (db repairs go through the daemon only —
   the pen tools, or `gm_hook call <MESSAGE_TYPE>`; never touch the
   sqlite file).
