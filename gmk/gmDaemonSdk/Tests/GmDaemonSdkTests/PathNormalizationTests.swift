...
final class PathNormalizationTests: XCTestCase {
    ...
    /// Calls `RepoRelativePath` directly rather than going through the
    /// `Store` forwarder. The normalizer MOVED down into this module during the
    /// package split — it is lexical, Foundation-only, and was misfiled in the
    /// persistence target. Reaching it through Store here would mean this test
    /// forced the base layer to depend on the middle one.
    private func normalize(_ raw: String, root: String? = nil) throws -> String {
        try RepoRelativePath.normalizeRepoRelativePath(raw, repoRoot: root ?? self.root)
    }
