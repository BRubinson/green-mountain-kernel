            ...
            .onAppear {
                ...
                sink.onZoom = { factor, anchor in
                    viewState.viewport.zoom(
                        to: viewState.viewport.zoom * factor,
                        anchor: anchor
                    )
                }
            }
            ...
            .onChanged { value in
                ...
                case .erase:
                    ...
                    if case .element(let element)? = workspace.resolved.hitTest(
                        at: p,
                        edgeTolerance: 6 / viewState.viewport.zoom,
                        includeInk: true
                    ) {
                        switch element.kind {
                        case .stroke, .shape:
