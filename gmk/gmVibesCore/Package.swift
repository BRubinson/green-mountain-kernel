...
let package = Package(
    ...
    dependencies: [
        // EXACTLY THREE. The first two were established by counting every import
        // under the old gmk/gmVibes/: 67 files import GmDaemonSdk, 17 import
        ...
        //
        // THE THIRD EDGE, GmITerm2Client, is what the run bar's Play button
        // launches through. Three things about it that would otherwise be
        // re-derived wrongly:
        //
        //   - The app talks to iTerm2 DIRECTLY. The kernel is not in that path
        //     and gains no verb from this; nothing new goes on the wire.
        //   - That package is NONISOLATED-by-default while this one is
        //     MainActor-by-default, and the asymmetry is DELIBERATE — its
        //     socket work must never be pinned to the main actor. See its own
        //     manifest header.
        //   - SwiftProtobuf stays BEHIND that boundary. Nothing in this package
        //     imports it, and nothing generated by protobuf crosses the
        //     transport's actor boundary to get here.
