    ...
    public struct ResolvedElementView: View {
        ...
        public init(
            element: ResolvedElement, environment: DiagramRenderEnvironment,
            offset: CGSize = .zero
        ) {
        ...
        public var body: some View {
            ZStack(alignment: .topLeading) {
                ForEach(element.children, id: \.uuid) { child in
                    ResolvedElementView(
                        element: child, environment: environment,
                        offset: offset)
                }
            }
    ...
    struct ShapeView: View {
        ...
        var body: some View {
            Canvas { context, _ in
                ...
                context.stroke(
                    path, with: .color(Color(hex: shape.strokeColor)),
                    style: StrokeStyle(
    ...
    public struct UmlNodeView: View {
        ...
        public init(
            element: ResolvedElement, node: ResolvedUmlNode,
            offset: CGSize = .zero
        ) {
            self.element = element
            self.node = node
    ...
    public struct DopeScopeOutlineView: View {
        ...
        public init(
            element: ResolvedElement, card: ResolvedScopeCard?,
    ...
    public struct DopeEntityCardView: View {
        ...
        public init(
            element: ResolvedElement, model: EntityCardModel?,
            ...
            Canvas { context, _ in
                ...
                for edge in edges {
                    ...
                    case .dopeForeignKey:
                        ...
                        context.stroke(
                            path, with: .color(.secondary.opacity(0.7)),
                            style: StrokeStyle(lineWidth: 1.2))
                        context.fill(
                            Path(
                                ellipseIn: CGRect(
                                    x: edge.to.x - 2.5, y: edge.to.y - 2.5,
                                    width: 5, height: 5)),
