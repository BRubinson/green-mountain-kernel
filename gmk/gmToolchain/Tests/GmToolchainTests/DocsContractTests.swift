...
///
/// Three of these also walk every package's `Sources`. That widening is the point: the
...
final class DocsContractTests: XCTestCase {
    ...
    /// plugins/gmcc/, resolved by MARKER through the shared helper.
    ///
    ...
    /// REPO-relative, not plugin-relative. The walks now span `plugins/gmcc/`
    /// AND five packages under `gmk/`, so a path stripped against the plugin
    /// root would leave gmk paths absolute — and every allowlist entry keyed on
    /// the stripped form would silently stop matching, turning each exemption
    /// into a permanent failure.
    private func relative(_ url: URL) -> String {
        url.path.replacingOccurrences(of: RepoRoot.resolve().path + "/", with: "")
    }

    /// Every package's Sources tree — the CLI help text, cheatsheet body and
    /// doc comments that a `.md`-only walk misses.
    ///
    /// FIVE trees now, not one. The old single `daemon/Sources` walk would
    /// still "work" after the split in the sense of not crashing, while
    /// covering none of the code — which is the failure this whole helper
    /// exists to make impossible.
    private func swiftFiles() throws -> [URL] {
        let out = RepoRoot.swiftSources()
        XCTAssertGreaterThan(out.count, 20, "source tree walk looks broken")
        return out
    }
    ...
    func testNoRetiredEnvNames() throws {
        let hits = try violations(
            ...
            allowFiles: [
                "plugins/gmcc/skills/gmcc/SKILL.md",             // the retirement notice itself
            ])
    ...
    func testZshrcBlockMarkerOnlyInCleanupDocs() throws {
        let hits = try violations(
            pattern: #">>> gmcc env >>>"#,
            allowFiles: [
                "plugins/gmcc/commands/gm_init.md",                 // "check it does NOT exist" + pointer
                "plugins/gmcc/skills/gmcc_cleanup_system/SKILL.md", // the deletion remedy
            ])
    ...
    func testAdoptFlagAbsentFromBotTiers() throws {
        for name in ["plugins/gmcc/commands/gm_bot.md", "plugins/gmcc/commands/gm_bot_rpi.md", "plugins/gmcc/commands/gm_bot_team.md"] {
    ...
    func testNoRetiredSessionBaseTier() throws {
        ...
        hits += try violations(
            ...
            allowFiles: [
                ...
                "gmk/gmDaemonSdk/Sources/GmDaemonSdk/Dope/DopeLevel.swift",
                // Documents the retired spelling it is tolerant of.
                "gmk/gmDaemonSdk/Sources/GmDaemonSdk/Protocol/Rows.swift",
            ],
            allowLine: allowLine)
    ...
    ///
    /// Walks the doc tree, the agent defs AND every package's `Sources`: the retired
