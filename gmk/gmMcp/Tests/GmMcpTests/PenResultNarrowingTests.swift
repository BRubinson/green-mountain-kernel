import XCTest
import GRDB
// @testable on BOTH, and for the reason @testable exists: this test drives a
// real Store to build its fixture (`store.dbQueue`, internal to GmDaemon) and
// encodes through the SDK's internal `AnyEncodable`. Neither is API the pen
// should publish — a `public dbQueue` would hand every consumer a second door
// to the single-writer database.
@testable import GmDaemon
@testable import GmDaemonSdk
@testable import gm_mcp
