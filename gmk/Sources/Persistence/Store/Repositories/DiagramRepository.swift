...
struct DiagramOwner {
    ...
    /// DERIVED, not an ownership tier: a session's instance, resolved
    /// through the join rather than stored on the diagram row.
...
struct DiagramRepository: RepositoryContext {
    ...
    /// cannot reference another table, and ALTER TABLE ADD COLUMN cannot add
    /// one. The rule is deliberately asymmetric. On WRITE a binding resolving to
    ...
    func fetchDiagramTree(diagram: DiagramRow) throws -> DiagramTree {
        ...
        return DiagramTree(
            ...
            instanceUuid: diagram.instanceUuid,
        ...
        func walk(_ node: DiagramElementNode) throws {
            if case .dopeScopePersistenceLayer(let payload) = node.payload {
                ...
                if let sessionUuid = diagram.sessionUuid {
                    ...
                    scope =
                        try dope.dopeProjectScopeCandidates(
                            projectUuid: diagram.projectUuid,
    ...
    func diagramBatchApply(_ req: DiagramBatchApplyRequest) throws -> DiagramBatchApplyResponse {
        ...
        try recordDiagramChange(
            ...
            mutationCount: req.mutations.count,
            revision: revision
        )
    ...
    private func applyDiagramRowUpdate(
        ...
        // under GMFS storage, which a project has as much as a session does.
    ...
    func insertSubtypeRow(
        ...
        case .dopeScopePersistenceLayer(let p):
            _ = try core.insertBase(
                db,
                table: "diagram_dope_scope_persistence_layer",
                extra: [
                    "element_uuid": elementUuid,
    ...
    private func replaceSubtypeRow(
        ...
        func requireRow(_ table: String) throws {
            guard db.changesCount > 0 else {
                throw StoreError.corruptState(
                    entity: table,
                    detail: "element \(elementUuid) has no subtype row"
                )
            }
        }
        switch payload {
        case .drawingLayer(let p):
            try db.execute(
                sql: """
                    ...
                    """,
                arguments: [
                    p.opacity, p.visible ? 1 : 0, p.locked ? 1 : 0,
                    now, elementUuid,
                ]
            )
        ...
        case .drawingStroke(let p):
            try db.execute(
                sql: """
                    ...
                    """,
                arguments: [
                    p.tool.rawValue, p.strokeColor, p.strokeWidth,
        ...
        case .drawingShape(let p):
            try db.execute(
                sql: """
                    ...
                    """,
                arguments: [
                    p.shapeKind.rawValue, p.strokeColor, p.strokeWidth,
        ...
        case .drawingText(let p):
            try db.execute(
                sql: """
                    ...
                    """,
                arguments: [
                    p.markdown, p.width, p.height, p.fontSize,
        ...
        case .connector(let p):
            try db.execute(
                sql: """
                    ...
                    """,
                arguments: [
                    p.targetElementUuid, p.strokeColor, p.strokeWidth,
        ...
        case .umlNode(let p):
            try db.execute(
                sql: """
                    ...
                    """,
                arguments: [
                    p.nodeKind.rawValue, p.width, p.height, p.markdown,
    ...
    private func replaceVertices(
        ...
        try db.execute(
            sql: "DELETE FROM \(table) WHERE \(parentColumn) = ?",
            arguments: [elementUuid]
        )
