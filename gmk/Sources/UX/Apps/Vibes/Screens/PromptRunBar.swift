...
private enum PluginPreflight {
    enum Result: Equatable {
        ...
        /// installed plugin, as it always has.
        case notConfigured
        /// An environment root with no plugin in any clone. This one blocks:
        /// emitting no `--plugin-dir` here would load the installed marketplace
        /// plugin silently, which is the failure this preflight exists to make loud.
        case directoryMissing(path: String)
        case manifestMissing(path: String)
        case versionMismatch(pluginVersion: String, appVersion: String)
        case ok
    }
    ...
    private static func evaluate() -> Result {
        ...
        let appVersion = GmVersion.current
        guard pluginVersion == appVersion else {
            return .versionMismatch(pluginVersion: pluginVersion, appVersion: appVersion)
        }
        return .ok
    }
}
...
struct PromptRunBar: View {
    ...
    enum Block: Equatable {
        ...
        case noRoot
        case kernelMissing(root: String)
        case noRepo
        case pluginDirMissing(path: String)
        case pluginManifestMissing(path: String)
        case pluginVersionMismatch(pluginVersion: String, appVersion: String)
    }
    ...
    private var block: Block? {
        ...
        let kernel = URL(fileURLWithPath: root, isDirectory: true)
            .appendingPathComponent("bin", isDirectory: true)
            .appendingPathComponent("gm_kernel")
        guard FileManager.default.isExecutableFile(atPath: kernel.path) else {
            return .kernelMissing(root: root)
        }
        ...
        case .versionMismatch(let pluginVersion, let appVersion):
            return .pluginVersionMismatch(pluginVersion: pluginVersion, appVersion: appVersion)
        }
        return nil
    }
    ...
    private func blockCopy(_ block: Block) -> String {
        ...
        case .kernelMissing(let root):
            return
                "No staged kernel at \(root)/bin/gm_kernel — a pane on this root has nothing to dial. Seed the environment (`bash gmk/scripts/gm_env.sh seed <env>`; ⌘R does it for test)."
        case .noRepo:
            return "This instance has no resolved checkout on disk."
        case .pluginDirMissing(let path):
            return
                "No environment plugin under \(path). `claude --plugin-dir` REPLACES the installed plugin and there is no fallback, so launching without it would silently run the installed plugin instead of this environment's. Seed the environment (`bash gmk/scripts/gm_env.sh seed <env>`; ⌘R does it for test), then relaunch GMVibes."
        case .pluginManifestMissing(let path):
            return
                "No plugin manifest at \(path)/.claude-plugin/plugin.json. `claude --plugin-dir` REPLACES the installed plugin with no fallback, so a pane launched now would load a plugin serving none of the tools its agents are granted. Re-seed the environment, then relaunch GMVibes."
        case .pluginVersionMismatch(let pluginVersion, let appVersion):
            return
                "The plugin at \(PluginPreflight.directory ?? "—") is \(pluginVersion); this kernel is \(appVersion). Re-seed the environment (`bash gmk/scripts/gm_env.sh seed <env>`; ⌘R does it for test), then relaunch GMVibes."
        }
    }
