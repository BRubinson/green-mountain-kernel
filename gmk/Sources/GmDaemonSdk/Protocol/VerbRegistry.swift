...
/// verbs` and by the suite, instead of several drifting copies. Every
/// `MessageType` must be here or in an explicit allowlist.
///
...
enum VerbRegistry {
    ...
    static let compositeCdeTools: [String: [MessageType]] = [
        ...
        // `pen:` values because `VerbSpec.cdeTool` is one name per verb — which
        ...
        // value for the same reason as the searches above — `cdeTool` is one
    ...
    static let all: [VerbSpec] = [
        ...
        VerbSpec(
            ...
            aliases: ["gm hook subagent-start"],
            role: .record(agentPhases: nil)
        ),
        ...
        // spelling, registered as an alias so the generated PreToolUse deny set
        // can see it: an agent typing it by hand would forge capture rows for a
        // tool call that never happened. NO `pen:` NAME, DELIBERATELY — capture
        // happens ONLY as the PostToolUse hook, and a pen door here is that
        // same forgery offered as a typed tool. Do not "complete" this row in a
        // coverage sweep.
        VerbSpec(
            .fileChangeAdd,
        ...
        VerbSpec(.fileChangeList, gm: "gm file-change list", cde: "cde_search_file_changes", role: .read),
        ...
        VerbSpec(.kbiteMawOpen, gm: "gm kbite maw-open", cde: "kbite_open_maw", role: .record(agentPhases: nil)),
        VerbSpec(.kbiteDigest, gm: "gm kbite digest", cde: "kbite_digest", role: .record(agentPhases: nil)),
        VerbSpec(.kbiteGet, gm: "gm kbite get", role: .read),
        VerbSpec(.kbiteFileGet, gm: "gm kbite file-get", role: .read),
        VerbSpec(.kbiteSearch, gm: "gm kbite search", cde: "kbite_search", role: .read),
        ...
        // ── Search ───────────────────────────────────────────────────────
        VerbSpec(.catalogSearch, gm: "gm catalog search", cde: "projects_search", role: .read),
        ...
        VerbSpec(
            .clarifyOpen,
            gm: "gm clarify open",
            cde: "rpir_open_clarification",
            role: .record(agentPhases: [.clarifyOpen])
        ),
        VerbSpec(
            .clarifyQuestionAdd,
            gm: "gm clarify question-add",
            cde: "rpir_write_clarification_questions",
            role: .record(agentPhases: [.clarifyOpen])
        ),
        VerbSpec(
            .clarifyNoteAdd,
            gm: "gm clarify note-add",
            cde: "rpir_write_clarification_notes",
            role: .record(agentPhases: [.clarifyOpen])
        ),
        VerbSpec(
            .clarifySeal,
            gm: "gm clarify seal",
            cde: "rpir_seal_clarification",
            role: .record(agentPhases: [.clarifyOpen])
        ),
        VerbSpec(
            .clarifyAnswer,
            gm: "gm clarify answer",
            cde: "rpir_answer_clarification_question",
        ...
        VerbSpec(
            ...
            cde: "rpir_finalize_clarification",
            role: .record(agentPhases: [.clarifyUser])
        ),
        VerbSpec(.clarifyGet, gm: "gm clarify get", cde: "rpir_get_clarification", role: .read),

        // ── Care package ─────────────────────────────────────────────────
        VerbSpec(
            .carePackageOpen,
            gm: "gm clarify package-open",
            cde: "rpir_open_care_package",
            role: .record(agentPhases: [.carePackage])
        ),
        VerbSpec(
            .carePackageRefAdd,
            gm: "gm clarify package-add",
            cde: "rpir_write_care_package",
            role: .record(agentPhases: [.carePackage])
        ),
        ...
        VerbSpec(
            ...
            cde: "rpir_close_care_package",
            role: .record(agentPhases: [.carePackage])
        ),
        VerbSpec(.carePackageGet, gm: "gm clarify package-get", cde: "rpir_get_care_package", role: .read),

        // ── Architecture machine ─────────────────────────────────────────
        VerbSpec(
            .archOpen,
            gm: "gm arch open",
            cde: "rpir_open_architecture",
            role: .record(agentPhases: [.architecture])
        ),
        VerbSpec(
            .archSummarize,
            gm: "gm arch summarize",
            cde: "rpir_summarize_architecture",
            role: .record(agentPhases: [.architecture])
        ),
        VerbSpec(
            .archPersistAdd,
            gm: "gm arch persist-add",
            cde: "rpir_write_architecture_persistence_changes",
            role: .record(agentPhases: [.architecture])
        ),
        VerbSpec(
            .archFieldAdd,
            gm: "gm arch field-add",
            cde: "rpir_write_architecture_field_changes",
            role: .record(agentPhases: [.architecture])
        ),
        VerbSpec(
            .archGeneralAdd,
            gm: "gm arch general-add",
            cde: "rpir_write_architecture_general_changes",
            role: .record(agentPhases: [.architecture])
        ),
        VerbSpec(
            .archOptionAdd,
            gm: "gm arch option-add",
            cde: "rpir_open_architecture_option",
        ...
        VerbSpec(
            .archDecide,
            gm: "gm arch decide",
            cde: "rpir_decide_architecture",
            role: .record(agentPhases: [.archOptions])
        ),
        VerbSpec(
            .archPropose,
            gm: "gm arch propose",
            cde: "rpir_propose_architecture",
            role: .record(agentPhases: [.architecture])
        ),
        VerbSpec(
            .archApprove,
            gm: "gm arch approve",
            cde: "rpir_approve_architecture",
            role: .record(agentPhases: [.planGate])
        ),
        VerbSpec(
            .archRevise,
            gm: "gm arch revise",
            cde: "rpir_revise_architecture",
            role: .record(agentPhases: [.planGate])
        ),
        VerbSpec(.archGet, gm: "gm arch get", cde: "rpir_get_architecture", role: .read),
        ...
        VerbSpec(
            ...
            cde: "rpir_complete_exploration",
            role: .record(agentPhases: [.explore, .clarifyOpen])
        ),
        VerbSpec(.exploreReopen, gm: "gm explore reopen", role: .record(agentPhases: nil)),
        VerbSpec(.exploreGet, gm: "gm explore get", cde: "rpir_get_exploration", role: .read),

        // ── Review machine ───────────────────────────────────────────────
        VerbSpec(.reviewOpen, gm: "gm review open", cde: "rpir_open_review", role: .record(agentPhases: [.review])),
        VerbSpec(
            .reviewFindingAdd,
            gm: "gm review finding-add",
            cde: "rpir_write_reviews",
        ...
        VerbSpec(
            ...
            cde: "rpir_load_exploration_brief",
            role: .read
        ),
        ...
        VerbSpec(.dopeSearch, gm: "gm dope search", cde: "dope_search_session", role: .read),
        ...
        VerbSpec(.dopeWriteRepo, gm: "gm dope write-repo", cde: "dope_update_session", role: .record(agentPhases: nil)),
