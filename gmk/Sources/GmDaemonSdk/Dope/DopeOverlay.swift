...
enum DopeOverlay {
    ...
    enum Origin: String, Codable, Hashable, Sendable {
        ...
        case tombstoned
        /// The overlay has a node at a path absent from the base. Legal and
    ...
    private static func singleLayerResolve(_ tree: DopeScopeTree, origin: Origin) -> Resolved {
        ...
        for domain in tree.domains {
            ...
            if isTombstone(domain.identity) {
                ...
                resolutions[dPath] = Resolution(
                    ...
                    baseUuid: nil,
                    overlayUuid: domain.identity.uuid
                )
                continue
            }
            resolutions[dPath] = Resolution(
        ...
        return Resolved(
            tree: tree.replacingDomains(domains),
    ...
    private static func merge(base: DopeScopeTree, overlay: DopeScopeTree) -> Resolved {
        ...
        let overlayDomains = Dictionary(
            overlay.domains.map { ($0.body.code, $0) },
        ...
        // whiteouts over something absent from the base.
        ...
        return Resolved(
            tree: base.replacingDomains(domains),
            resolutions: resolutions,
            hidden: hidden,
            warnings: warnings
        )
    ...
    private static func mergeDomainChildren(
        ...
        overlay: DopePersistenceNode?,
        resolutions: inout [String: Resolution],
        hidden: inout [String]
    ) -> ([DopeEntityNode], [DopeEnumNode]) {
        let ovEntities = Dictionary(
            (overlay?.entities ?? []).map { ($0.body.code, $0) },
            uniquingKeysWith: { a, _ in a }
        )
        let ovEnums = Dictionary(
            (overlay?.enums ?? []).map { ($0.body.code, $0) },
            uniquingKeysWith: { a, _ in a }
        )
        ...
        for be in base.entities {
            ...
            resolutions[path] = Resolution(
                path: path,
                origin: ov == nil ? .base : .overridden,
                effectiveUuid: use.identity.uuid,
                baseUuid: be.identity.uuid,
                overlayUuid: ov?.identity.uuid
            )

            let ovProps = Dictionary(
                (ov?.properties ?? []).map { ($0.body.code, $0) },
                uniquingKeysWith: { a, _ in a }
            )
            ...
            for bp in be.properties {
                ...
                resolutions[pPath] = Resolution(
                    path: pPath,
                    origin: op == nil ? .base : .overridden,
                    effectiveUuid: useP.identity.uuid,
                    baseUuid: bp.identity.uuid,
                    overlayUuid: op?.identity.uuid
                )
                props.append(useP)
        ...
        for bn in base.enums {
            ...
            for bo in bn.options {
                ...
                resolutions[oPath] = Resolution(
                    path: oPath,
                    origin: oo == nil ? .base : .overridden,
                    effectiveUuid: useO.identity.uuid,
                    baseUuid: bo.identity.uuid,
                    overlayUuid: oo?.identity.uuid
                )
                options.append(useO)
