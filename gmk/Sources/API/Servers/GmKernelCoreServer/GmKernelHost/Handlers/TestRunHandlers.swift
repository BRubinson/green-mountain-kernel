...
enum TestSuiteListHandler {
    /// Handles TEST_SUITE_LIST requests.
    ///
    /// - Parameters:
...
enum TestLockStatusHandler {
    /// Handles TEST_LOCK_STATUS requests.
    ///
    /// - Parameters:
    ///   - line: The encoded request payload.
    ///   - head: The message envelope header.
...
///
/// Reclaims a lock whose holder is provably gone.
enum TestLockAcquireHandler {
    /// Handles TEST_LOCK_ACQUIRE requests.
    ///
    /// - Parameters:
    ///   - line: The encoded request payload.
    ///   - head: The message envelope header.
...
enum TestLockReleaseHandler {
    /// Handles TEST_LOCK_RELEASE requests.
    ///
    /// - Parameters:
    ///   - line: The encoded request payload.
    ///   - head: The message envelope header.
...
/// TEST_RUN_START — a claimed run actually began.
///
/// Split from acquire because claiming and starting are different moments, and
/// "claimed but never started" is a state worth being able to see.
enum TestRunStartHandler {
    /// Handles TEST_RUN_START requests.
    ///
    /// - Parameters:
    ///   - line: The encoded request payload.
    ///   - head: The message envelope header.
...
enum TestRunStatusHandler {
    /// Handles TEST_RUN_STATUS requests.
    ///
    /// - Parameters:
    ///   - line: The encoded request payload.
    ///   - head: The message envelope header.
