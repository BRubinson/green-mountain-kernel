...
extension GmBridgeScript {
    ...
    /// SessionStart provisioning.
    ///
    /// Stays a FILE and stays a `command` hook: the harness accepts only
    /// `command` and `mcp_tool` on SessionStart, an `mcp_tool` handler
    /// there is documented to expect a "not connected" error on first run,
    /// and SessionStart is precisely where the claude-session binding every
    /// later write depends on gets created.
    ...
    ///
    /// Cannot become a kernel subcommand for the obvious reason: it is what
    /// PUTS the kernel on the machine. It runs before any binary exists.
    ...
    /// cannot drift from the authored one.
    ///
    /// See `GmBridgeScript+Bodies.swift`.
