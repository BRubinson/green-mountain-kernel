...
struct TestRunRepository: RepositoryContext {
    ...
    func suiteList(_ req: TestSuiteListRequest) throws -> TestSuiteListResponse {
        guard let instance = try InstanceRecord.fetchOne(
            db, sql: """
    ...
    func acquire(_ req: TestLockAcquireRequest) throws -> TestLockResponse {
        ...
        let runUuid = try core.insertBase(db, table: "test_run", now: now, extra: [
        ...
        let expiresAt: String? = holderKind == .lease
            ? StoreCore.isoNow(offsetSeconds: req.leaseSeconds ?? 3600)
            : nil

        try core.updateBase(db, table: "project_test_lock", uuid: cell.uuid,
                            expectedVersion: cell.version, set: [
        ...
        try core.appendEvent(db, kind: .testLockChange, subjectUuid: cell.uuid,
    ...
    func release(_ req: TestLockReleaseRequest) throws -> TestLockResponse {
        ...
        try finish(runUuid: req.runUuid, state: req.finalState,
    ...
    func runStart(_ req: TestRunStartRequest) throws -> TestRunResponse {
        try core.updateBase(db, table: "test_run", uuid: req.runUuid,
                            expectedVersion: req.expectedVersion, set: [
    ...
    private func fetchOrOpenCell(projectUuid: String) throws -> ProjectTestLockRecord {
        ...
        let uuid = try core.insertBase(db, table: "project_test_lock", extra: [
    ...
    private func requireCell(uuid: String) throws -> ProjectTestLockRecord {
        guard let row = try ProjectTestLockRecord.fetchOne(
    ...
    private func abandon(runUuid: String) throws {
        try finish(runUuid: runUuid, state: .abandoned, exitCode: nil,
