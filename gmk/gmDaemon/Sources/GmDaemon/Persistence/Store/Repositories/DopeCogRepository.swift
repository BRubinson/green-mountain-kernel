...
struct DopeCogRepository: RepositoryContext {
    ...
    func dopeCogAdd(_ req: DopeCogAddRequest) throws -> DopeCogResponse {
        ...
        try dope.recordDopeChange(
            scope: scope, action: "cog_add", level: nil,
    ...
    func dopeCogDelete(_ req: DopeCogDeleteRequest) throws -> DopeCogDeleteResponse {
        ...
        try dope.recordDopeChange(
            scope: scope, action: "cog_delete", level: nil,
            nodeUuid: req.uuid, revision: revision)
        return DopeCogDeleteResponse(
            deletedUuid: req.uuid, cascadedElements: elements,
    ...
    func dopeCogElementAdd(
        ...
        guard
            try Row.fetchOne(
                db, sql: "SELECT uuid FROM dope_cog WHERE uuid = ?",
                arguments: [req.cogUuid]) != nil
        else {
        ...
        try dope.recordDopeChange(
            scope: scope, action: "cog_element_add", level: nil,
            nodeUuid: uuid, revision: revision)
    ...
    func dopeCogElementUpdate(
        ...
        if !set.isEmpty {
            try core.updateBase(
                db, table: "dope_cog_element", uuid: req.uuid,
                expectedVersion: req.expectedVersion, set: set)
        }
        let revision = try dope.bumpScopeRevision(
        ...
        try dope.recordDopeChange(
            scope: scope, action: "cog_element_update", level: nil,
            nodeUuid: req.uuid, revision: revision)
    ...
    func dopeCogElementDelete(
        ...
        let revision = try dope.bumpScopeRevision(
            ...
            ownerUuid: cogUuid)
        try dope.recordDopeChange(
            scope: scope, action: "cog_element_delete", level: nil,
            nodeUuid: req.uuid, revision: revision)
        return DopeCogDeleteResponse(
            deletedUuid: req.uuid, cascadedElements: children,
            scopeUuid: scope.uuid, revision: revision)
    }
