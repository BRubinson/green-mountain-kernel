...
struct DiagramRepository: RepositoryContext {
    ...
    func recordDiagramChange(
        ...
        try core.appendEvent(
            db, kind: .diagramChange, subjectUuid: diagram.uuid,
    ...
    func resolveDiagramOwner(
        ...
        case .session:
            ...
            return DiagramOwner(
                tier: .session, projectUuid: row["project_uuid"],
        ...
        case .prompt:
            ...
            return DiagramOwner(
                tier: .prompt, projectUuid: row["project_uuid"],
                instanceUuid: row["instance_uuid"],
    ...
    func fetchDiagramTree(diagram: DiagramRow) throws -> DiagramTree {
        ...
        return DiagramTree(
            identity: DopeNodeIdentity(
                uuid: diagram.uuid, version: diagram.version,
                ...
                if let sessionUuid = diagram.sessionUuid {
                    if let promptUuid = diagram.promptUuid {
                        scope = try dope.dopeScopeCandidates(
                            ...
                            promptUuid: promptUuid, code: payload.dopeScopeCode
                        ).first
    ...
    func fetchElementInfo(uuid: String) throws -> ElementRowInfo {
        ...
        return ElementRowInfo(
            uuid: row["uuid"], diagramUuid: row["diagram_uuid"],
    ...
    private func applyElementUpdate(
        ...
        try core.updateBase(
            db, table: "diagram_element", uuid: info.uuid,
        ...
        return DiagramMutationResult(
            index: index, kind: "element_update",
    ...
    private func applyElementDelete(
        ...
        return DiagramMutationResult(
            index: index, kind: "element_delete",
    ...
    private func applyDiagramRowUpdate(
        ...
        try core.updateBase(
            db, table: "diagram", uuid: diagram.uuid,
            expectedVersion: update.expectedVersion, set: set)
        guard
            let version = try Int64.fetchOne(
        ...
        return DiagramMutationResult(
            index: index, kind: "diagram_update",
    ...
    func insertSubtypeRow(
        ...
        case .drawingLayer(let p):
            _ = try core.insertBase(
                db, table: "diagram_drawing_layer",
                extra: [
        ...
        case .drawingStroke(let p):
            ...
            _ = try core.insertBase(
                db, table: "diagram_drawing_stroke",
                extra: [
                    "element_uuid": elementUuid,
        ...
        case .drawingShape(let p):
            _ = try core.insertBase(
                db, table: "diagram_drawing_shape",
                extra: [
                    "element_uuid": elementUuid,
        ...
        case .dopeScopePersistenceLayer(let p):
            _ = try core.insertBase(
                db, table: "diagram_dope_scope_persistence_layer",
                extra: [
                    "element_uuid": elementUuid,
